...
export const leadFormSchema = z.object({
  name: z.string().trim().min(2, "Escribí tu nombre completo.").max(120, "El nombre es demasiado largo."),
  phone: z
    .string()
    .trim()
    .min(8, "Escribí un teléfono válido.")
    .max(30, "El teléfono es demasiado largo."),
  location: z.string().trim().min(2, "Escribí tu ubicación.").max(160, "La ubicación es demasiado larga."),
  billRange: z.enum(billRangeValues, { message: "Seleccioná un rango de factura." }),
  interest: z.enum(interestValues, { message: "Seleccioná qué te interesa." }),
