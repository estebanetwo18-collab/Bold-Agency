            ...
            {pending.map((t, i) => (
              <Reveal key={i} delay={(confirmed.length + i) * 80}>
                <div className="flex h-full flex-col items-center justify-center rounded-3xl border border-dashed border-border-strong p-8 text-center">
                  <p className="text-xs font-semibold uppercase tracking-wide text-text-faint">{t.name}</p>
                  <p className="mt-2 text-xs text-text-faint">Espacio reservado para un testimonio adicional</p>
                </div>
              </Reveal>
            ))}
          </div>
