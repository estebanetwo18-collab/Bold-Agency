...
export async function POST(request: NextRequest) {
  ...
  // Honeypot: un bot llenó un campo que ningún humano ve. Se responde 200
  // "vacío" para no delatar la trampa, pero no se procesa nada.
  if (values.website) {
  ...
  if (!webhookConfigured) {
    console.warn(
      "[lead] LEAD_WEBHOOK_URL no está configurado — modo de prueba local. " +
        "El lead se guarda en .data/leads.local.jsonl y NO se envía a Excel. " +
        "Ver README.md → 'Conectar con Excel Online'.",
    );
  ...
  if (webhookResult.attempted && !webhookResult.ok) {
    ...
    // Respaldo local para no perder el lead mientras se resuelve el webhook.
    await persistLeadLocally(lead);
    return NextResponse.json(
      {
        ok: false,
        message:
          "No pudimos registrar tu solicitud en este momento. Tus datos no se perdieron, por favor intenta de nuevo.",
      },
      { status: 502 },
    );
  }
